        ...
        try {
            ...
            if (Math.abs(System.currentTimeMillis() - date) >= 31536000000L) {
                ...
                if (dayDiff == 0 || dayDiff == -1 && System.currentTimeMillis() - date < 60 * 60 * 8 * 1000) {
                    return ConfigManager.getBooleanOrFalse(Defines.showSecondsInTimestamp) ? getInstance().formatterDayWithSeconds.format(new Date(date))
                        : getInstance().formatterDay.format(new Date(date));
